...
SHIPPED_ROLE = [{
    ...
    "actions": SECURITY_AUDIT_BEDROCK | {
        ...
        # SecurityAudit grants guardduty:Get*/List* — the THREAT section has always
        # relied on that, and slice 2.6 relies on the same grant for the AI Protection
        # findings THREAT's own severity filter excludes. Added here because the fixture
        # was incomplete, not because the requirement is new.
...
# Slice 2.2 added nine more, all for AgentCore -- a SEPARATE service whose IAM
# prefix is bedrock-agentcore. SecurityAudit predates it entirely, so it grants
# none of them, and without them a whole agent estate is invisible rather than
...
def test_the_shipped_role_is_missing_exactly_eightytwo_actions():
...
def test_the_blocked_checks_are_the_knowledge_base_and_guardrail_grading_ones():
    ...
    led = L.evaluate(SHIPPED_ROLE)
    assert set(led.blocked) == {"AGC-01", "AGC-02", "AGC-03", "AGC-04", "AGC-05", "AGC-06", "AGC-07", "AGC-08", "AGT-03", "AGT-04", "AGY-01", "AGY-02", "AGY-03", "AIGRD-01", "AIGRD-02", "AIGRD-04", "AILOG-04", "AILOG-05", "AILOG-06", "AMEM-02", "MART-01", "MCP-01", "MCP-02", "MCP-03", "MCP-04", "S3T-01", "S3T-02", "LATT-01", "LATT-02", "CART-01", "DIRSVC-01", "DIRSVC-02", "AMP-01", "XRAY-01", "GLUE-01", "GLUE-02", "LSAIL-01", "LSAIL-02", "NFW-01", "NFW-02", "NFW-03", "PCA-01", "QS-01", "SSO-01", "CB-01", "CB-02", "DOCDB-01", "DOCDB-02", "DOCDB-03", "EMR-01", "EMR-02", "IMGB-01", "IOT-01", "IOT-02", "IOT-03", "XFER-01", "XFER-02", "XFER-03", "PERIM-01", "PERIM-02", "PERIM-03", "SHAI-01", "SHAI-02", "TFLOW-01", "VEC-01", "VEC-02", "VEC-03", "VEC-04", "VEC-05", "VEC-06", "VEC-07"}
    assert led.blocked["AGT-03"] == ("bedrock:GetDataSource",
                                     "bedrock:GetKnowledgeBase")
    # Slice 3.1: GetDataSource now also buys TFLOW-01, because the data
    # source TYPE is what separates a proven untrusted-content path from
    # an assumed one.
    assert "bedrock:GetDataSource" in led.blocked["TFLOW-01"]
...
def test_the_additive_policy_contains_only_read_actions():
    ...
    for action in pol["Statement"][0]["Action"]:
        ...
        # BatchGet* is a read verb like the rest -- aoss:BatchGetCollection is how that
