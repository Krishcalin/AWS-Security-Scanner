...
| Seam | Lambda artifact fetch (function zip + layer zips → `LMB-07`) | `aws_sidescan_lambda.fetch_lambda_artifact` (injected `http_get`; the shipped default is `aws_layer_fetch.http_get`, the same allowlisted egress the ECR layer pull uses — no new seam) | operator's AWS (presigned S3 URL) | **opt-in** (`--side-scan-lambda`) | in-VPC S3 endpoint |
