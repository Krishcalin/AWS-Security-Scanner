...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/checks-509%20severity--mapped-red?style=flat-square" alt="509 severity-mapped checks"/>
...
- [Live Audit Scanner (`aws_live_scanner.py`)](#live-audit-scanner-aws_live_scannerpy)
  ...
  - [Security Checks Coverage (509 severity-mapped checks across 95 sections)](#security-checks-coverage-509-severity-mapped-checks-across-95-sections)
...
|---------|------|------|-------|--------|
| **OverWatch** (Live CNAPP) | `aws_live_scanner.py` | Live AWS API audit + security graph + attack-path CNAPP | Running AWS account (multi-account via AssumeRole) | 509 severity-mapped across 95 sections |
...
- **509 severity-mapped checks** across 95 audit sections (all 509 actionable, each with a detailed remediation write-up)
...
```bash
# Run full audit (all 95 sections, 509 severity-mapped checks)
...
### Security Checks Coverage (509 severity-mapped checks across 95 sections)
...
│   ├── aws_live_scanner.py         # Live Audit Scanner v3.0.0 (95 sections, graph, exposure+L7, deep-plane, correlate, effperm, state, ciem, sidescan, KSPM/KIEM, flow-logs, backends, remediate, codetocloud, finding-detail, engine-EOL, winvuln, DSPM, secrets, least-priv, AI-SPM, CDR, forensics, copilot, vuln-ingest, supply-chain, registry)
