...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/checks-523%20severity--mapped-red?style=flat-square" alt="523 severity-mapped checks"/>
...
- [Live Audit Scanner (`aws_live_scanner.py`)](#live-audit-scanner-aws_live_scannerpy)
  ...
  - [Security Checks Coverage (523 severity-mapped checks across 96 sections)](#security-checks-coverage-523-severity-mapped-checks-across-96-sections)
...
|---------|------|------|-------|--------|
| **OverWatch** (Live CNAPP) | `aws_live_scanner.py` | Live AWS API audit + security graph + attack-path CNAPP | Running AWS account (multi-account via AssumeRole) | 523 severity-mapped across 96 sections |
...
- **523 severity-mapped checks** across 96 audit sections (all 523 actionable, each with a detailed remediation write-up)
...
```bash
# Run full audit (all 96 sections, 523 severity-mapped checks)
...
### Security Checks Coverage (523 severity-mapped checks across 96 sections)
...
│   ├── aws_live_scanner.py         # Live Audit Scanner v3.0.0 (96 sections, graph, exposure+L7, deep-plane, correlate, effperm, state, ciem, sidescan, KSPM/KIEM, flow-logs, backends, remediate, codetocloud, finding-detail, engine-EOL, winvuln, DSPM, secrets, least-priv, AI-SPM, CDR, forensics, copilot, vuln-ingest, supply-chain, registry)
