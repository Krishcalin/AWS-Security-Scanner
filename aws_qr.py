...
Written rather than depended on because the wheelhouse is pinned and arch-portable
for air-gapped installs (see `requirements.txt`), and a QR library is not worth
spending that budget on. It exists for exactly one job: showing an `otpauth://` URI
that Microsoft Authenticator, Google Authenticator, Authy and 1Password can scan.
