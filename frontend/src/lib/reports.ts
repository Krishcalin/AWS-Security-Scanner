...
import { capNote, ofTotal } from './cap'
...
export function buildExecHtml(opts: {
  ...
  // A report is read as complete by someone who cannot go and check, so every
  // cut in it is stated: the table caption carries "N of M", and a row whose
  // affected list was trimmed says how many it did not show.
  const severe = findings.filter((f) => f.severity === 'CRITICAL' || f.severity === 'HIGH')
  const TOP_N = 25
  const topRows = severe.slice(0, TOP_N)
  const topCaption = ofTotal(topRows.length, severe.length)
  const top = topRows
    .map((f) => {
      const aff = f.affected || []
      const more = capNote(3, aff.length)
      const affCell = _esc(aff.slice(0, 3).join(', ')) + (more ? ` <em>(${_esc(more)})</em>` : '')
      return `<tr><td class="mono">${_esc(f.check_id)}</td><td>${_esc(f.severity)}</td>`
        + `<td>${_esc(f.section)}</td><td>${_esc(f.risk)}</td>`
        + `<td class="mono">${affCell}</td></tr>`
    }).join('')
...
<h2>Top critical &amp; high findings <span style="font-weight:400;color:#64748b;font-size:13px">(showing ${_esc(topCaption)})</span></h2>
