...
- **Finding Detail** (`aws_finding_detail.py`) -- pure offline data module: `FINDING_DETAIL={check_id:{risk, impact, steps[...]}}` for all 572 check IDs (100% of `REMEDIATION_MAP`); the detailed risk / business-impact / step-by-step remediation the JSON (`finding_catalog`) and HTML (per-finding cards) reports render. Falls back to the one-line `REMEDIATION_MAP` CLI for any uncatalogued check
...
│   ├── aws_finding_detail.py       # Finding detail — risk/impact/step-by-step remediation for all 572 checks, pure offline data (GENERATED)
...
Every scan emits, for each of the 572 checks, a full write-up — the
