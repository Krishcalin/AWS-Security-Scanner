...
- **Finding Detail** (`aws_finding_detail.py`) -- pure offline data module: `FINDING_DETAIL={check_id:{risk, impact, steps[...]}}` for all 587 check IDs (100% of `REMEDIATION_MAP`); the detailed risk / business-impact / step-by-step remediation the JSON (`finding_catalog`) and HTML (per-finding cards) reports render. Falls back to the one-line `REMEDIATION_MAP` CLI for any uncatalogued check
...
- **AI-SPM** (`aws_aispm.py`) -- pure classifiers for the blast radius of an AI resource's execution role (`role_privesc_capable` / `role_reaches_crown` [a graph query over the `CAN_READ_DATA` edges DSPM emits] / `ai_network_exposed` / `is_ai_crown`); fused onto the graph POST-clobber in the DATA section → `AISPM-01..03` + the conditional `AIPATH-01` (egress + role reach, no ingress claim), with `aws_correlate.py` byte-unchanged
- **AI-posture scanner sections** — `VECTORSTORE` (VEC-*, who can reach the agent's
  corpus and who holds its key), `SHADOW_AI` (SHAI-*, AI built by a role nobody
  declared, in a region nobody scans) and `AI_LOGGING` (AILOG-*, where the prompts
  land once BDR-01's advice is taken). All three were dispatchable and absent from
  `SECTIONS`, the default run list, so no scan ran them and their names appeared in
  no document — 14 checks nobody could reach. `tests/test_section_reachability.py`
  is the ratchet that stops a section being dispatchable and unrun again; it caught
  `SIDESCAN` in the same state, which had made `--side-scan` a flag nothing acted on.
...
- **Web Console** (`frontend/`) -- React 19 + Vite + TS + Tailwind v4 SPA over the hub API: Overview / Attack Paths (interactive React Flow graph) / Findings / Vulnerabilities / **Supply Chain** (SBOM diff · license policy · VEX) / Cloud Accounts + onboarding wizard / Inventory / Identity / Compliance / Remediation / Reports / Settings. **Scope + every open panel live in the URL** (shareable/bookmarkable deep links via `lib/deeplink.ts`), and an **interactive product tour** (`lib/tour/`) replays 5 canned scenarios over the live console. Runs on engine-shaped sample fixtures (zero AWS) or the live hub. **Vitest** covers the deep-link codecs + a fixture-grounded scenario guardrail

- **Ownership attribution** (`aws_ownership.py`) -- pure Application entity + three-tier attribution (explicit resource pin > tag selector > whole-account claim); a tier matching two applications stops as AMBIGUOUS rather than falling through, and `AttributionCoverage` travels with the buckets so a scorecard cannot render a subset as a total
...
- **Guardrail gate decisions** (`aws_guardrail.py`) -- pure CI/CD verdict layer: an unavailable gate does what its strictest configured mode would have done, every permissive path is a degraded allow or an attributed break-glass override (both exit 2, never a pass), and a pipeline timeout is an incomplete evaluation rather than consent. **Wired into the IaC gate** (`aws_offline_scanner --policy --environment --break-glass-* --gate-record`), where it replaced an `except PolicyError: continue  # inert policy` that dropped an unparseable policy and exited 0 — D5's own defect, in the one gate this product ships into other people's pipelines
- **Trend & forecast sufficiency** (`aws_trend.py`) -- pure time-series with sufficiency counted in USABLE periods rather than elapsed time; below three usable periods no projected value is produced at all (the observed series still is). Accuracy tracking refuses on its own terms until enough predictions resolve. **Wired to `GET /accounts/{id}/forecast`** via `monthly_series`, which emits a month with no scan as an explicit GAP rather than skipping it — skipping is the tempting implementation and it is how a series that is mostly hole reports itself production-grade
- **Credential-exposure ingest** (`aws_ingest_credexp.py`) -- pure vendor-neutral breach-corpus normaliser joining leaked identities to cloud principals. No network calls, credential material never enters the product (allowlist-built records + salted digests), and a hit is recorded as an observation of a corpus, never as a compromise. **Wired to `CREDEXP-00..03`** in the IAM section via `--cred-exposures --cred-exposure-salt --estate-domains`, with `iam:ListAccessKeys` collected so the leaked-key join can actually run. CREDEXP-00 states what could NOT be determined, because with no key inventory an empty result means "we could not look" and reads as "you are clean"

### Phase II governance layer (v2.38.0) — SRS review + the six defects

`OW2-SRS-001 v0.1` (the OverWatch Phase II SRS) was reviewed against this codebase.
Two things came out of it.

**The specification misjudges its own difficulty.** Of nine functional modules, two
were already substantially shipped in Phase I — FR-7 Digital Twin (scheduled *last*,
in II-C) and FR-9 Identity/CIEM — while FR-2 Scorecards, scheduled earlier, started
from nothing. And AD-02, filed as an *assumption*, is the load-bearing floor under
FR-2, FR-6, `OW2-CC-012` filtering and every by-owner KRA: findings carry an account,
and an account is not an owner. That is why `aws_ownership.py` was the first slice.

**Six defects in the specification are fixed here**, each with ratifiable replacement
text under `docs/`. They share one failure mode in six costumes — *a number that
quietly narrows what it was computed over, and reads as good news for doing so*:

| | Defect | Fix | Document |
|---|---|---|---|
| D1 | Appendix B weights summed to 95, and a maximally bad estate earned an **A** | normalise in the engine; risk ascends, posture descends, the grade bands the posture | `docs/RISK_MODEL.md` |
| D2 | compensating controls credited into the **exposure** component | credit `findings` only, gated on live exposure; effective ceiling ~4.7 pts, published | `docs/RISK_MODEL.md` |
| D3 | nothing distinguished *clean* from *never looked at* | interval + three verdicts; `NOT_ESTABLISHED` in its own column | `docs/KRA_METRICS.md` |
| D4 | one approval moved the headline KRA from **0% to 100%** | adjusted and unadjusted rates render together; two counter-KRAs targeting zero | `docs/KRA_METRICS.md` |
| D5 | FR-5 never said what a blocking gate does when it is **down** | strictest configured mode decides; every permissive path is exit 2, never a pass | `docs/GUARDRAIL_FAILURE_MODE.md` |
| D6 | FR-4 scheduled to begin the month its data does | refuse rather than label; sufficiency counts *usable* periods | `docs/FORECAST_SEQUENCING.md` |

Proposed requirements for ratification: `OW2-KM-007`, `OW2-KM-001(f)/(g)`,
`OW2-GR-008/009/010`, the FR-4a/FR-4b split, and amendments to Appendix B,
`OW2-KM-004`, `OW2-GR-005` and the II-C gate.

**Known un-wired — the list is EMPTY, and that is the state to defend.** It held four
modules: tested, documented, safe code that nothing outside its own tests imported.
A bullet in this file is how an un-wired module comes to be counted as working
software, so all four were wired rather than described:

| Module | Now reached by | What the recorded blocker turned out to be |
|---|---|---|
| `aws_trend.py` | `cnapp_service.get_forecast` → `GET /accounts/{id}/forecast` | **Stale.** "Needs trend materialisation" — `StateStore.trend` already had the series and `scan_coverage` already had the coverage denominator. The gap was an adapter from scans to the calendar months AD-04 counts in, plus the judgement of what coverage is measured against. |
| `aws_guardrail.py` | the IaC gate in `aws_offline_scanner.main` | **Half stale.** `ExposureGate` gained a producer in `aws_factors.exposure_gate`; the real gap was a CI entry point, and `aws_offline_scanner` already was one. It also *had D5's defect in it*: `except PolicyError: continue  # inert policy` dropped an unparseable policy and exited 0. |
| `cnapp_marketplace_metering.py` | `cnapp_worker.meter_marketplace_usage` | Its own docstring said "the caller schedules it hourly" and nothing did. The worker is the only hourly process. Three gates (product code, client factory, metering store), all off by default. |
| `aws_ingest_credexp.py` | `CREDEXP-00..03` in the IAM section | Needed an ingest surface **and** `iam:ListAccessKeys` — one decision, not two, exactly as `docs/PRODUCTION.md` said: without a live key inventory the highest-value join in the corpus cannot happen. |

`tests/test_unreached_modules.py` is the ratchet: a NEW module that nothing imports
fails the build, an entry that gains a caller also fails, and — since `aws_nhi` —
merely *importing* a module does not count as reaching it, because registration is
not execution. To shrink the list, wire the module or delete it with its tests;
adding an entry is not progress. Four modules left it this way and two earlier:
`aws_sidescan_lambda` when its producer was written (`LMB-07` was ticked complete in
the vuln roadmap while registered in none of the four metadata maps and emitted by no
code path, so the check could not fire), and `cnapp_worker` when it gained
`python -m hub.cnapp_worker` and a caller in `build_service` — which also had to stop
an unwired `session_factory` marking a customer's account `denied`.
...
**All three of this slice's library-only modules are now wired** — `aws_trend` to
`GET /accounts/{id}/forecast`, `aws_guardrail` to the IaC gate, `aws_ingest_credexp`
to `CREDEXP-00..03` with `iam:ListAccessKeys` collected. Each still withholds rather
than assumes: the forecast produces no number below three usable months, the gate
never reports an incomplete evaluation as a pass, and a corpus hit is recorded as an
observation of a corpus rather than as a compromise.
...
│   ├── aws_cis_al2.py              # CIS Amazon Linux 2 v4.0.0 — the 35 of 287 recommendations SSM Inventory can decide (AL2-01/02/03) + the coverage statement, pure
│   ├── aws_cis_al2_map.py          # The AL2 mapping AS DATA — all 287 rows by data source, and the CAPABILITY each blocked row waits on. No titles reproduced
│   ├── aws_cis_db2.py              # CIS AWS Database Services — tranche 2: the 13 recorded gaps, 9 built (DDB-06/07, ELC-09, MDB-07, DOCDB-09/10, NEP-11, TS-03/04) and 4 proved unbuildable, pure
...
│   ├── aws_finding_detail.py       # Finding detail — risk/impact/step-by-step remediation for all 587 checks, pure offline data (GENERATED)
...
- **Slice 3 — AI-SPM + CDR-lite streaming ingest + cloud-forensics timeline** (v2.26.0):
  `aws_aispm.py` (`AISPM-01/02/03` + conditional `AIPATH-01`, emitted post-clobber in DATA);
...
Every scan emits, for each of the 587 checks, a full write-up — the
...
- **CHECK_MAP**: Dict mapping 102 section names -> bound check methods
- **102 sections**: ORGANIZATIONS, IAM, S3, VPC, LOGGING, CLOUDWATCH, KMS, EC2, AMI, ECR, BACKUP, RDS, GLACIER, SNS, SQS, CLOUDFRONT, ROUTE53, BEDROCK, BEDROCK_AGENTS, AGENTCORE, LAMBDA, EKS, ECS, SECRETS, WAF, ELASTICACHE, OPENSEARCH, DYNAMODB, STEPFUNCTIONS, APIGATEWAY, ELB, EBS, REDSHIFT, EFS, ACM, SAGEMAKER, COGNITO, APIGATEWAYV2, IAMPRIVESC, EXPOSURE, COGNITO_IDENTITY, SIDESCAN, WINVULN, AMAZONLINUX, VULN, THREAT, DATA, AI_THREAT, AI_LOGGING, SHADOW_AI, VECTORSTORE, IOT, EMR, CODEBUILD, DOCDB, NEPTUNE, MEMORYDB, TIMESTREAM, IMAGEBUILDER, TRANSFER, NETWORKFIREWALL, LIGHTSAIL, PRIVATECA, QUICKSIGHT, IDENTITYCENTER, GLUE, S3TABLES, VPCLATTICE, CODEARTIFACT, DIRECTORYSERVICE, PROMETHEUS, XRAY, LAKEFORMATION, WORKSPACESWEB, STORAGEGATEWAY, PAYMENTCRYPTO, MANAGEDBLOCKCHAIN, WORKMAIL, SITEWISE, IOTMANAGEDINT, MAILMANAGER, CODEGURUPROFILER, VERIFIEDPERMISSIONS, CLOUDHSM, CLOUDWAN, MANAGEDGRAFANA, AURORADSQL, FLEETWISE, CLOUDFORMATION, FIREWALLMANAGER, ECRPUBLIC, MULTIPARTYAPPROVAL, WICKR, MEDIAPACKAGE, APPRUNNER, BATCH, BEANSTALK, DRS, WORKSPACES, APPSTREAM, NHI, CORRELATE
- **Three lockstep maps** (a check_id lands in all three when actionable): `CHECK_SEVERITY` (587 entries), `COMPLIANCE_MAP` (587), `REMEDIATION_MAP` (587). All three maps now hold the same 587 keys, so every check carries a compliance mapping, a one-line remediation and a full `aws_finding_detail.FINDING_DETAIL` write-up. "Actionable" is NOT the same as "proven to fail": `docs/CHECK_FIRING.md` records that the suite drives 445 of them to a FAIL, 62 emit only WARN/INFO/PASS, and 28 are never observed at all — measured by recording `_add` across a real run, because it cannot be read off the source. COMPLIANCE_MAP NIST values stay inside a FROZEN 38-control universe (`compliance/crosswalk.json`), asserted by `tests/test_compliance_crosswalk.py`
...
### CIS AWS Database Services Benchmark v2.0.0 — the gap list is now empty

Mapped in `engine/aws_cis_db_map.py`, built across `engine/aws_cis_db.py` (tranche 1) and
`engine/aws_cis_db2.py` (tranche 2). `docs/CIS_DATABASE_BENCHMARK.md` and
`docs/CIS_DATABASE_INVENTORY.md` are **GENERATED**; never hand-edit them.

Tranche 1 recorded thirteen recommendations as decidable-but-unbuilt. Tranche 2 built
**nine** of them and proved the other **four are not buildable at all** — which is the more
useful half, because a gap nobody can close is a backlog item that never leaves:

* **5.8 / 5.9** are misfiled in the benchmark: filed under ElastiCache, audited against
  Amazon **Keyspaces**, whose control-plane reads this product deliberately declines
  because `cassandra:Select` also reads table rows.
* **6.4** (MemoryDB audit logging) is console-only. The pinned botocore model has no
  log-delivery member on `Cluster` and no log-delivery shape in the service at all.
* **10.8** is a process control — its audit reads "Define Monitoring Objectives".

**Read the model, not the docs.** MemoryDB's member is `SnsTopicArn`, *not* `SNSTopicArn`.
Checking the shipped botocore model caught that before MDB-07 shipped; the assumed spelling
would have read `None` on every cluster and failed all of them — a check that is always
right for entirely the wrong reason.

### CIS Amazon Linux 2 Benchmark v4.0.0 — and why only 35 of 287 are checks

Mapped in `engine/aws_cis_al2_map.py`; checks in `engine/aws_cis_al2.py`;
`docs/CIS_AL2_BENCHMARK.md` is **GENERATED** by `scripts/cis_al2_benchmark.py`.

**This is an operating-system benchmark and OverWatch is an agentless control-plane
scanner.** Those two facts decide the whole shape of it. Classifying all 287
recommendations by the state their own audit commands read:

| file content | file modes | accounts | kernel | packages | services |
|---:|---:|---:|---:|---:|---:|
| 125 | 41 | 21 | 46 | 37 | 7 |

Only packages and services are visible from outside the instance, through **SSM
Inventory** — the one read-only path to in-guest state this product has. Reading a *file*
would need `ssm:SendCommand`, which executes code and is a write.

**So 35 are decided and 252 are BLOCKED — not "gap".** The distinction is deliberate and
the mapping enforces it: a `gap` in the sibling mappings is a backlog item somebody could
build this afternoon, whereas every row here waits on a named capability that does not
exist. `BLOCKERS` records what would lift each one; the big one is
`aws_sidescan_fs.DissectExtractor`, which raises `SideScanUnavailable` on purpose and
gates 199 rows.

**No check was registered for the other 252.** That would have been the largest block of
registered-but-unreachable ids in this product's history — exactly what
`docs/CHECK_FIRING.md` measures and `tests/test_unreached_modules.py` keeps at zero.

**AL2-03 is rated above AL2-01** because an installed package is latent surface and an
enabled unit is a socket listening now. **AL2-00** is an INFO-only coverage statement
(unregistered, like `CREDEXP-00`): every check returns nothing when its inventory is
missing, and nothing is otherwise indistinguishable from a clean host.

**The mapping reproduces no recommendation titles at all** — rows are number plus
classification. A stronger copyright position than the three sibling mappings needed, and
a test enforces it.
...
In-guest OS posture: **AL2-XX** (CIS Amazon Linux 2, via SSM Inventory) beside **WINVULN-XX** (Windows, via SSM Patch Manager). Both assess inside an instance without an agent, and both report a stated coverage gap rather than a pass when the host is not inventoried.
...
```bash
python -m pytest tests/ -v         # 5745 tests, no AWS credentials needed
```
