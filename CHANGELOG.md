...
**this benchmark is materially weaker than its three siblings.** CIS marks every one of
the 56 "Manual" — not a single Automated control, where Foundations, Compute and
Database all carry substantial Automated sets. Seven recommendations have **no audit
procedure at all**: the heading is present and the body is empty. Around thirty have no
remediation text. And two would **reduce security if followed**.

- **`engine/aws_cis_storage_map.py`** — the mapping, as data. 56 rows, one per
  recommendation, under nine verdicts. Two of those nine exist because no sibling
  mapping needed them:
  - **`UNSAFE` (2 rows).** 2.2's audit procedure directs the reader to open SSH, HTTP
    and HTTPS to anywhere — the exact state `SEG-01`, `SEG-06` and `VPC-05` FAIL on.
    3.5 directs the reader to restrict EFS mount-target ingress to port 22, which is
    SSH; EFS serves NFS on 2049, so a group built to that instruction blocks every
    legitimate mount and admits nothing useful to an attacker. Neither will be built,
    and `test_no_check_ever_cites_an_unsafe_recommendation` makes citing either a build
    failure — the row is recorded so nobody files it as backlog in a year's time.
  - **`NO_AUDIT` (7 rows).** A hole in the source, which is neither a gap of ours nor a
    decision of ours, and saying so is more honest than either.
  - The rest: 7 `covered`, 13 `decided elsewhere` (generic IAM recommendations filed
    under "EBS"), 10 `gap` — the real backlog — 5 `blocked` on in-guest filesystem
    reads, 5 `process`, 5 `vacuous`, 2 `declined`.
- **`DRS-01..04` — Elastic Disaster Recovery, a new section.** Section 6 is the only
  part of the document naming settings a scanner can read, and what it points at is
  worth having: a staging subnet holding a continuous, byte-for-byte copy of every
  protected disk — production data living outside production, under a template nobody
  administers alongside the workloads it shadows. `ebsEncryption` carries a real `NONE`
  value, so "all of it, unencrypted, where nobody looks" is a state the API will report
  if asked, and nothing in OverWatch asked until now. Also public replication routing
  (`DRS-02`), stalled replication (`DRS-03`), and whether recovery has **ever** been
  exercised (`DRS-04`, from `lastLaunchResult`, which answers benchmark 6.8 directly
  rather than by inference).
- **`BCK-04`/`BCK-05` — the two questions the benchmark never asks.** Its AWS Backup
  section asks repeatedly that backups be *created* and never once whether they survive
  the loss of the region or the compromise of the account. `BCK-04` fails a plan whose
  rules carry no `CopyActions`; `BCK-05` fails a vault on an AWS-owned key, which the
  account cannot audit, rotate or revoke. Both deliberately carry **no `CIS-STORAGE`
  key** — that absence is the finding, and a test pins it.
- **`engine/aws_cis_storage.py`** — the decisions, pure: no boto3, no clock, every
  branch unit-testable. Every field name and enum was read off the pinned botocore
  model rather than recalled. In-flight replication states (`INITIAL_SYNC`, `BACKLOG`,
  `RESCAN`, …) are deliberately **not** failures: a server catching up is working, and
  failing it would make every freshly-added server a finding for as long as the first
  sync takes.
- **`CIS-STORAGE` is its own compliance key**, not a fold into `CIS`, because the
  section numbers collide — "2.1" means a different thing in each document and one key
  would silently merge two claims. The key lands only on a check named by a `covered`
  row *and* belonging to that row's own service section: `EC2-17` is named by 2.4 and
  keeps its `CIS-COMPUTE` citation, so a benchmark mapping cannot quietly re-label
  checks belonging to other pillars.
- **`docs/CIS_STORAGE_BENCHMARK.md`** — generated by `scripts/cis_storage_benchmark.py`
  from the mapping, never hand-edited. As with its three siblings, the source PDF is
  **not** in this repository and no title, rationale, audit or remediation text is
  reproduced; recommendation *numbers* are references.
- **The ratchet moved as designed.** 566 → 572 registered checks, and proven-failing
  508 → 514: exactly +6, because the six arrived **with** their driving tests. That is
  the first batch to land since the never-observed ceiling was set to 0, and it is what
  the ceiling was for — there is no backlog for a check to hide in, so the two numbers
  move together in one commit or the build fails.
