...
POSTGRES_DDL: List[str] = [
    ...
    # ── multi-tenancy plane (Phase-4 Slice-1) — BIGINT epoch twins ──────────────
    """CREATE TABLE IF NOT EXISTS workspaces(
       workspace_id TEXT PRIMARY KEY, name TEXT NOT NULL DEFAULT '', slug TEXT,
       status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','suspended','archived')),
       plan TEXT, created_at BIGINT NOT NULL, updated_at BIGINT NOT NULL)""",
    ...
    # v11: local authentication. Twin of the sqlite DDL — see aws_state.py for why the
    # session table holds a token FINGERPRINT rather than the token.
    ...
    # v12: TOTP second factor. Separate tables rather than columns on app_user — see
    # aws_state.py for why (IF NOT EXISTS adds tables, never columns).
    ...
    """CREATE TABLE IF NOT EXISTS usage_events(
       ...
       period TEXT NOT NULL, event_epoch BIGINT NOT NULL, event_iso TEXT NOT NULL,
